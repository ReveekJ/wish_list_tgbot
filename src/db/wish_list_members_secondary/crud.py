...
class WishListMembersSecondaryCRUD(DBTemplate[WishListMembersModel, WishListMember]):
    ...
    def does_pair_exists(self, user_id: int, wishlist_id: int) -> bool:
        query = select(self._model_class.user_id).where(self._model_class.user_id == user_id,
                                                        self._model_class.wishlist_id == wishlist_id)
    ...
    def get_wish_lists_id(self, user_id: int) -> List[int]:
        ...
        res: list[WishListMember] = [self._schema_class.model_validate(i, from_attributes=True) for i in
                                     self._db.execute(query).scalars().all()]
    ...
    def delete_pair(self, user_id: int, wishlist_id: int) -> Optional[WishListMembersModel]:
        obj = self._db.query(self._model_class).filter(self._model_class.user_id == user_id,
                                                       self._model_class.wishlist_id == wishlist_id).first()
        if obj:
            self._db.delete(obj)
            self._db.commit()
